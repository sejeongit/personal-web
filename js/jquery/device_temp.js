...
(function($){
  ...
  $.ajax({
    ...
    var browserSetFn = function(){
      ...
      var ckIndex;
      var useRwd = false;
      // IE브라우저 체크(ms엔진인 Trident엔진이 있을 경우 IE브라우저) : navigator.userAgent.search('Trident')
    ...
    // 101. 브라우저의 가로값의 변경만 파악하여 처리(resize는 가로, 세로 구분하지 않기 때문)
      ...
      */
      ...
      for( ; i >= 0 ; i-=1 ){
        ...
        if(winWidth >= intSize){
          ...
          // for문을 10000번 반복했을 경우, 조건이 100번째에 맞으면 100번까지만 하고 반복수행을 끝내라 해야 하지만
      ...
      return checkType; // checkType이 함수 안에 있는 변수이므로 return으로 밖으로 빼야함
    };

    var beforeWidth = deviceWidth();
    console.log('브라우저 변경 전 가로 크기: ', beforeWidth);
    // $('.device').hide();
    // $('.' + beforeWidth).show(); // beforeWidth는 위 deviceWidth 함수에서 return checkType으로 내보냈으므로 결과는 type임.
